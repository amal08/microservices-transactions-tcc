...
public class ContentController extends CompositeTransactionParticipantController {
    ...
    @ApiOperation(code = 200, value = "Find a source code item by its Id", notes = "Queries data previously persisted in the database", response = SourceCodeItem.class, produces = "application/json")
    public SourceCodeItem get(@ApiParam(value = "Id of the item to retrieve", required = true) @PathParam("id") String id) {
    ...
    @POST
    @ApiOperation(code = 201, value = "Save a new source code item in the database", notes = "The newly created resource can be referenced by the URI returned in the the Location header field", response = String.class, responseHeaders = {
            @ResponseHeader(name = "Location", description = "The URI of the saved item", response = String.class) })
    @ApiResponses(value = { @ApiResponse(code = 500, message = "Error saving the given item", response = String.class) })
    public Response save(@Context UriInfo uriInfo, @ApiParam(value = "Data of the item", required = true) SourceCodeItem content) {
        try {
            ...
            Response response = Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(new ExceptionRestHandler().toString(e)).type(MediaType.TEXT_PLAIN).build();
    ...
    @Path("{txid}/{id}")
    @Produces("application/json")
    @ApiOperation(code = 200, value = "Find a source code item by its Id", notes = "Queries the transaction uncommitted data in addition to the one previously persisted in the database", response = SourceCodeItem.class, produces = "application/json")
    public SourceCodeItem getTxAware(@ApiParam(value = "Id of a composite transaction", required = true) @PathParam("txid") String txid,
    ...
    @POST
    @Path("{txid}")
    @ApiOperation(code = 201, value = "Save a new source code item enlisting the operation in a composite transaction", notes = "No data will be persisted in the database until the transaction is explicitly committed. "
            + "The newly created resource can be referenced by the URI returned in the the Location header field", response = String.class, responseHeaders = {
                    @ResponseHeader(name = "Location", description = "The URI of the saved item", response = String.class) })
    @ApiResponses(value = { @ApiResponse(code = 500, message = "Error saving the given item", response = String.class) })
