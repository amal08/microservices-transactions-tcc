...
public class ForumController extends CompositeTransactionParticipantController {
    ...
    @ApiOperation(code = 200, value = "Find a discussion board by its Id", notes = "Queries data previously persisted in the database", response = Forum.class, produces = "application/json")
    public Forum get(@ApiParam(value = "Id of the discussion board to retrieve", required = true) @PathParam("id") String id) {
    ...
    @POST
    @ApiOperation(code = 201, value = "Save a new discussion board in the database", notes = "The newly created resource can be referenced by the URI returned in the the Location header field", response = String.class, responseHeaders = {
            @ResponseHeader(name = "Location", description = "The URI of the saved discussion board", response = String.class) })
    @ApiResponses(value = { @ApiResponse(code = 500, message = "Error saving the given discussion board", response = String.class) })
    ...
    @Path("{txid}/{id}")
    @Produces("application/json")
    @ApiOperation(code = 200, value = "Find a discussion board by its Id", notes = "Queries the transaction uncommitted data in addition to the one previously persisted in the database", response = Forum.class, produces = "application/json")
    public Forum getTxAware(@ApiParam(value = "Id of a composite transaction", required = true) @PathParam("txid") String txid,
    ...
    @POST
    @Path("{txid}")
    @ApiOperation(code = 201, value = "Save a new discussion board enlisting the operation in a composite transaction", notes = "No data will be persisted in the database until the transaction is explicitly committed. "
